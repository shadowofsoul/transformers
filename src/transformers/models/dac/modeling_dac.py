...
from ..deprecated._archive_maps import DAC_PRETRAINED_MODEL_ARCHIVE_LIST  # noqa: F401, E402
...
class DacConvTranspose1d(nn.Module):
...
class DacResnetBlock(nn.Module):
    """
...
class DacEncoder(nn.Module):
    ...
    def __init__(self, config: DacConfig):
        super().__init__()
        model = [DacConv1d(config, config.audio_channels, config.num_filters, config.kernel_size)]
        scaling = 1
        ...
        for ratio in reversed(config.upsampling_ratios):
            ...
            for j in range(config.num_residual_layers):
                model += [DacResnetBlock(config, current_scale, [config.dilation_growth_rate**j, 1])]
            # Add downsampling layers
            model += [nn.ELU()]
            model += [DacConv1d(config, current_scale, current_scale * 2, kernel_size=ratio * 2, stride=ratio)]
            scaling *= 2

        model += [DacLSTM(config, scaling * config.num_filters)]
        model += [nn.ELU()]
        model += [DacConv1d(config, scaling * config.num_filters, config.hidden_size, config.last_kernel_size)]
...
class DacDecoder(nn.Module):
    ...
    def __init__(self, config: DacConfig):
        ...
        model = [DacConv1d(config, config.hidden_size, scaling * config.num_filters, config.kernel_size)]
        ...
        for ratio in config.upsampling_ratios:
            ...
            model += [nn.ELU()]
            model += [
                DacConvTranspose1d(config, current_scale, current_scale // 2, kernel_size=ratio * 2, stride=ratio)
            ]
            # Add residual layers
            for j in range(config.num_residual_layers):
                model += [DacResnetBlock(config, current_scale // 2, (config.dilation_growth_rate**j, 1))]
            scaling //= 2

        # Add final layers
        model += [nn.ELU()]
        model += [DacConv1d(config, config.num_filters, config.audio_channels, config.last_kernel_size)]
        self.layers = nn.ModuleList(model)
...
class DacEuclideanCodebook(nn.Module):
